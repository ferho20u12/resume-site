...
export default function ExperienceCard({ experience }: ExperienceCardProps) {
  return (
    <div className="flex-none w-80 md:w-96 rounded-xl p-4 m-2 flex flex-col shadow-md bg-(--surface) border border-(--surface-border) text-(--text)">
      ...
      <div className="text-center mt-3">
        ...
        {experience.startDate && (
          <p className="text-xs text-(--text-soft)">{experience.startDate.toLocaleDateString()} - {experience.endDate ? experience.endDate.toLocaleDateString() : "Present"}</p>
        )}
        {experience.companyLink && (
          <a href={experience.companyLink} target="_blank" rel="noreferrer" className="text-xs text-(--accent) hover:text-(--accent-hover) hover:underline">Company site</a>
        )}
      </div>
