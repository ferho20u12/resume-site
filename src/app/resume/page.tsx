...
import { RESUME_DOWNLOAD_BUTTON_LABEL, RESUME_MOBILE_FALLBACK_TEXT, RESUME_PDF_BY_LANGUAGE, RESUME_PREVIEW_TITLE } from "@src/mock/resume.mock";
...
export default function ResumePage() {
  ...
  const title = RESUME_PREVIEW_TITLE[lang];

  return (
    <Card cardProps={{ nameSeccion: "resume" }}>
      <HorizontalScrollContainer title={title}>
        <div  className="flex flex-col gap-6 items-center w-full">
          <a href={pdfUrl} target="_blank" rel="noopener" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-(--surface-border) bg-(--surface) text-sm font-medium hover:bg-(--surface-hover) transition">
          ...
          <div className="md:hidden flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-sm text-(--text-soft) max-w-xs">{RESUME_MOBILE_FALLBACK_TEXT[lang]}</p>
            <a href={pdfUrl} target="_blank" rel="noopener" className="underline text-sm font-medium">
              {RESUME_DOWNLOAD_BUTTON_LABEL[lang]}
            </a>
          </div>
        </div>
